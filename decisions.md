...
**Status:** Code ready, smoke-tested. DINOv3 gated access request submit ho chuka hai (approval pending). `notebooks/train_swinir_dino_ablation_colab.ipynb` bhi ready hai — DINOv2 backbone se D028/D029 ke exact protocol (30 epochs, batch 8, lambda-perceptual 0.01, ICNR on) match karta hai taaki eval numbers directly comparable hon, plus ek commented-out cell already taiyaar hai DINOv3 sat493m checkpoint switch karne ke liye jab access approve ho jaaye. Real training run abhi baaki hai.
